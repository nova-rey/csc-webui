...
export default function Builder() {
  const { spec, setSpec, selected, setSelected, addProfile, removeProfile, addRule, removeRule } = useBuilderStore();
  const [normalized, setNormalized] = useState<unknown|null>(null);
  const [warnings, setWarnings] = useState<string[]>([]);
  const [errors, setErrors] = useState<string[]>([]);

  async function doNormalize() {
    setWarnings([]); setErrors([]); setNormalized(null);
    ...
    if (r.ok) {
      ...
      const detailsErrors = (r as unknown as {details?:{errors?:string[]}}).details?.errors ?? [];
  ...
  function exportJson(kind: "authoring"|"normalized") {
    ...
    a.href = url; a.download = kind === "authoring" ? "authoring_spec.json" : "spec_normalized.json";
  ...
  const currentRule = selected.kind==="rule" ? spec.behavior.rules.find(r=>r.id===selected.id) : undefined;
    ...
    <div className="grid lg:grid-cols-3 gap-4">
      <div className="bg-white p-3 rounded shadow">
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="flex gap-2">
            <button className="text-indigo-600 text-sm" onClick={()=>loadPreset("molly")}>Molly</button>
            <button className="text-indigo-600 text-sm" onClick={()=>loadPreset("contra")}>Contra</button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded shadow">
        ...
        {selected.kind==="identity" && (
          <IdentityForm value={spec.identity} onChange={(v)=>setSpec((s: AuthoringSpec)=>({...s, identity:v}))}/>
        )}
        {selected.kind==="table" && (
          <TableForm value={spec.table} onChange={(v)=>setSpec((s: AuthoringSpec)=>({...s, table:v}))}/>
        )}
        {selected.kind==="profile" && currentProfile && (
          <ProfileForm
            spec={spec}
            profile={currentProfile}
            onChange={(p)=>setSpec((s: AuthoringSpec)=>({...s, profiles: s.profiles.map((x)=>x.id===p.id?p:x)}))}
          />
        )}
        {selected.kind==="rule" && currentRule && (
          <RuleForm
            value={currentRule}
            onChange={(r)=>setSpec((s: AuthoringSpec)=>({
              ...s,
      ...
      <div className="space-y-3">
        <div className="bg-white p-3 rounded shadow">
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            <button className="bg-indigo-600 text-white px-3 py-1 rounded" onClick={doNormalize}>Normalize</button>
            ...
            <button className="border rounded px-3 py-1" onClick={()=>exportJson("authoring")}>Export Authoring</button>
            <button className="border rounded px-3 py-1" onClick={()=>exportJson("normalized")}>Export Normalized</button>
          </div>
        </div>
        ...
        {warnings.length>0 && (
          <div className="border border-yellow-200 bg-yellow-50 rounded p-2 text-sm">
            ...
            <ul className="list-disc list-inside text-yellow-800">{warnings.map((w,i)=><li key={i}>{w}</li>)}</ul>
          </div>
        )}
        <div className="bg-white p-3 rounded shadow">
