...
export function getMockApi() {
  return {
    async listSpecs(): Promise<ApiResp<SpecSummary[]>> {
      try {
        return ok(await load<SpecSummary[]>("/mock-data/specs.json"));
      } catch (e) {
    ...
    async normalizeSpec(_body: SpecNormalizeRequest): Promise<ApiResp<SpecNormalizeResult>> {
      void _body;
      try {
        return ok(await load<SpecNormalizeResult>("/mock-data/spec_normalize.json"));
      } catch (e) {
        return err((e as Error).message);
      }
    },
    async startRun(_payload: Record<string, unknown>): Promise<ApiResp<RunDetail>> {
      void _payload;
      try {
        return ok(await load<RunDetail>("/mock-data/run_detail.json"));
      } catch (e) {
        return err((e as Error).message);
      }
    },
    async listRuns(): Promise<ApiResp<RunRecord[]>> {
      try {
        return ok(await load<RunRecord[]>("/mock-data/runs.json"));
      } catch (e) {
        return err((e as Error).message);
      }
    },
