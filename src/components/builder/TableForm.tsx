    ...
    <div className="grid grid-cols-2 gap-3 text-sm">
      <label className="block">
        ...
        <input type="number" className="w-full border rounded px-2 py-1"
          value={value.min_bet ?? ""} onChange={e=>onChange({...value, min_bet: num(e.target.value)})}/>
      </label>
      <label className="block">
        <span className="block">Max Bet</span>
        <input type="number" className="w-full border rounded px-2 py-1"
          value={value.max_bet ?? ""} onChange={e=>onChange({...value, max_bet: num(e.target.value)})}/>
      </label>
      <label className="block col-span-2">
        <span className="block">Odds Profile</span>
        <select className="w-full border rounded px-2 py-1"
          value={(value.odds_profile ?? "3-4-5x") as string}
          onChange={e=>onChange({...value, odds_profile: e.target.value as OddsProfile})}>
